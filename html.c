...
void html_outtro(void)
{
  if (Houtro) fcat(Houtro);
    ...
    fprintf(outfile,"\t<p>For how I generate this, <a href=/guide/cs/web/werc/apps/search>see</a>\n");
...
int html_printfile(char *dirname, char *filename, struct _info *file, int descend)
{
    ...
    // Start the <a> tag for the file
    fprintf(outfile, "<a");
    
    // If the file structure is not null, apply class and title attributes
    if (file) {
        // Apply class based on the file type
        if (force_color) {
            fprintf(outfile, " class=\"%s\"", class(file));
        }
        
        // Add a title if there is a comment for the file
        if (file->comment) {
        ...
        // Generate the href part (links to the file)
        if (!nolinks) {
            fprintf(outfile, " href=\"/");

            // If dirname is provided, handle it as part of the URL
            if (dirname != NULL) {
                size_t len = strlen(dirname);
                size_t off = (len >= htmldirlen ? htmldirlen : 0);

                // Encode the directory and file names
                url_encode(outfile, dirname + (htmloffset ? off : 0));

                // If dirname is different from filename, append the filename
                if (strcmp(dirname, filename) != 0) {
                    if (dirname[strlen(dirname) - 1] != '/') {
                        putc('/', outfile);  // Add slash if necessary
                    }
                    url_encode(outfile, filename);
                }

                // Add the suffix based on descend level
                fprintf(outfile, "%s%s\"",
                        (descend > 1 ? "/00Tree.html" : ""),
                        (file->isdir && descend < 2 ? "/" : ""));
            } else {
                // If no dirname, just encode the filename
                url_encode(outfile, filename);
    ...
    // End the <a> tag and print the filename or host
    fprintf(outfile, ">");
    if (dirname) {
